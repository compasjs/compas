...
function internalQueryFile(builder = {}, wherePartial) {
  ...
  if (builder.viaGroup) {
    ...
    const offsetLimitQb = builder.viaGroup.offset
      ? query`OFFSET ${builder.viaGroup.offset}`
      : query``;
    if (builder.viaGroup.limit) {
      offsetLimitQb.append(
        query`FETCH FIRST ${builder.viaGroup.limit} ROWS ONLY`,
      );
    }
...
`;
  ...
  if (builder.viaGroupView) {
    builder.where = builder.where ?? {};
    const offsetLimitQb = builder.viaGroupView.offset
      ? query`OFFSET ${builder.viaGroupView.offset}`
      : query``;
    if (builder.viaGroupView.limit) {
      offsetLimitQb.append(
        query`FETCH FIRST ${builder.viaGroupView.limit} ROWS ONLY`,
      );
    }
    builder.where.idIn = query`
...
`;
  ...
  if (builder.group) {
    const joinedKeys = [];
    const offsetLimitQb = builder.group.offset
      ? query`OFFSET ${builder.group.offset}`
      : query``;
    if (builder.group.limit) {
      offsetLimitQb.append(query`FETCH FIRST ${builder.group.limit} ROWS ONLY`);
    }
...
${offsetLimitQb}
  ...
  if (builder.groupView) {
    const joinedKeys = [];
    const offsetLimitQb = builder.groupView.offset
      ? query`OFFSET ${builder.groupView.offset}`
      : query``;
    if (builder.groupView.limit) {
      offsetLimitQb.append(
        query`FETCH FIRST ${builder.groupView.limit} ROWS ONLY`,
      );
    }
...
function internalQueryFileGroup(builder = {}, wherePartial) {
  ...
  if (builder.viaFile) {
    builder.where = builder.where ?? {};
    const offsetLimitQb = builder.viaFile.offset
      ? query`OFFSET ${builder.viaFile.offset}`
      : query``;
    if (builder.viaFile.limit) {
      offsetLimitQb.append(
        query`FETCH FIRST ${builder.viaFile.limit} ROWS ONLY`,
      );
    }
...
`;
  ...
  if (builder.viaParent) {
    builder.where = builder.where ?? {};
    const offsetLimitQb = builder.viaParent.offset
      ? query`OFFSET ${builder.viaParent.offset}`
      : query``;
    if (builder.viaParent.limit) {
      offsetLimitQb.append(
        query`FETCH FIRST ${builder.viaParent.limit} ROWS ONLY`,
      );
    }
...
`;
  ...
  if (builder.viaChildren) {
    builder.where = builder.where ?? {};
    const offsetLimitQb = builder.viaChildren.offset
      ? query`OFFSET ${builder.viaChildren.offset}`
      : query``;
    if (builder.viaChildren.limit) {
      offsetLimitQb.append(
        query`FETCH FIRST ${builder.viaChildren.limit} ROWS ONLY`,
      );
    }
    builder.where.idIn = query`
...
`;
  ...
  if (builder.file) {
    const joinedKeys = [];
    const offsetLimitQb = builder.file.offset
      ? query`OFFSET ${builder.file.offset}`
      : query``;
    if (builder.file.limit) {
      offsetLimitQb.append(query`FETCH FIRST ${builder.file.limit} ROWS ONLY`);
    }
...
${offsetLimitQb}
  ...
  if (builder.parent) {
    const joinedKeys = [];
    const offsetLimitQb = builder.parent.offset
      ? query`OFFSET ${builder.parent.offset}`
      : query``;
    if (builder.parent.limit) {
      offsetLimitQb.append(
        query`FETCH FIRST ${builder.parent.limit} ROWS ONLY`,
      );
    }
...
${offsetLimitQb}
  ...
  if (builder.children) {
    const joinedKeys = [];
    const offsetLimitQb = builder.children.offset
      ? query`OFFSET ${builder.children.offset}`
      : query``;
    if (builder.children.limit) {
      offsetLimitQb.append(
        query`FETCH FIRST ${builder.children.limit} ROWS ONLY`,
      );
    }
 ...
 * @param {StoreFileQueryBuilder} builder
 */
function transformFile(values, builder) {
 ...
 * @param {StoreFileGroupQueryBuilder} builder
 */
function transformFileGroup(values, builder) {
  for (let i = 0; i < values.length; ++i) {
    let value = values[i];
    if (isPlainObject(value.result) && Object.keys(value).length === 1) {
      values[i] = value.result;
 ...
 * @param {StoreFileGroupViewQueryBuilder} builder
 */
function transformFileGroupView(values, builder) {
  for (let i = 0; i < values.length; ++i) {
    let value = values[i];
    if (isPlainObject(value.result) && Object.keys(value).length === 1) {
      values[i] = value.result;
 ...
 * @param {StoreJobQueryBuilder} builder
 */
function transformJob(values, builder) {
  for (let i = 0; i < values.length; ++i) {
    let value = values[i];
    if (isPlainObject(value.result) && Object.keys(value).length === 1) {
      values[i] = value.result;
 ...
 * @param {StoreSessionQueryBuilder} builder
 */
function transformSession(values, builder) {
  for (let i = 0; i < values.length; ++i) {
    let value = values[i];
    if (isPlainObject(value.result) && Object.keys(value).length === 1) {
      values[i] = value.result;
