...
const strings = [ `${tableName}"createdAt", ${tableName}"updatedAt", ${tableName}"id" ` ];
...
${it.bucketName ?? null}, ${it.contentLength ?? null}, ${it.contentType ?? null}, ${it.name ?? null}, ${JSON.stringify(it.meta ?? {})}, ${it.createdAt ?? (new Date())}, ${it.updatedAt ?? (new Date())}, ${it.deletedAt ?? null}
)`);
...
q.append(query`(
${options?.includePrimaryKey ? query`${it.id}, ` : undefined}
${it.isComplete ?? false}, ${it.priority ?? 0}, ${it.scheduledAt ?? (new Date())}, ${it.name ?? null}, ${JSON.stringify(it.data ?? {})}, ${it.createdAt ?? (new Date())}, ${it.updatedAt ?? (new Date())}
)`);
if (i !== insert.length - 1) {
q.append(query`, `);
}
...
q.append(query`(
${options?.includePrimaryKey ? query`${it.id}, ` : undefined}
${it.expires ?? null}, ${JSON.stringify(it.data ?? {})}, ${it.createdAt ?? (new Date())}, ${it.updatedAt ?? (new Date())}
)`);
if (i !== insert.length - 1) {
q.append(query`, `);
}
