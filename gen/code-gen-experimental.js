...
export function extendWithCodeGenExperimental(generator) {
  ...
  generator.add(
    ...
    T.object("anyDefinition").keys({
      ...
      rawValueImport: T.object().keys({
        javaScript: T.string().optional(),
        typeScript: T.string().optional(),
      }),
      rawValidator: T.string().optional(),
      rawValidatorImport: T.object().keys({
        javaScript: T.string().optional(),
        typeScript: T.string().optional(),
      }),
    }),
    T.object("anyOfDefinition").keys({
      type: "anyOf",
      ...namedTypeDefinitionBase,
      values: T.array()
        .min(1)
    ...
    T.object("booleanDefinition").keys({
      type: "boolean",
      ...namedTypeDefinitionBase,
    ...
    T.object("extendDefinition").keys({
      type: "extend",
      ...namedTypeDefinitionBase,
      keys: T.generic()
        .keys(T.string())
    ...
    T.object("genericDefinition").keys({
      type: "generic",
      ...namedTypeDefinitionBase,
      keys: T.reference("experimental", "typeSystemDefinition"),
      values: T.reference("experimental", "typeSystemDefinition"),
    }),
    T.object("numberDefinition").keys({
      type: "number",
      ...namedTypeDefinitionBase,
    ...
    T.object("referenceDefinition").keys({
      type: "reference",
      ...typeDefinitionBase,
      reference: T.object().keys({
        group: namePart,
        name: namePart,
      }),
    }),
    T.object("relationDefinition").keys({
      type: "relation",
    ...
    T.object("routeInvalidationDefinition").keys({
      type: "routeInvalidation",
      target: T.object().keys({
        group: namePart,
        name: T.optional().value(namePart),
      }),
      properties: T.object().keys({
